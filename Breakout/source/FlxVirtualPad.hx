...
public class FlxVirtualPad extends FlxGroup
{
	...
	public static final int RIGHT_BREAK = 5;
	...
	public FlxVirtualPad(int DPad, int Action)
	{
		...
		switch(Action)
		{
			...
			case A_B_X_Y:
				...
				break;
			case RIGHT_BREAK:
				actions.add(add(buttonA = createButton(FlxG.width - 50, FlxG.height - 68, 37, 37, ImgButtonRight)));
				actions.add(add(buttonBreak = createButton(FlxG.width - 50, FlxG.height - 130, 37, 37, ImgButtonA)));
				break;
	...
	public void destroy()
	{
		...
		if(actions != null)
			actions.destroy();
		dPad = actions = null;
		buttonA = buttonB = buttonC = buttonY = buttonX = buttonBreak= null;
